...
export default class Expertise extends Component{
    render(){
        ...
        const expertiseList= expertise.length ?(expertise.map(exp=>{
                            return(
                                <div className="md:w-1/2 rounded overflow-hidden shadow-lg hover:shadow-xl my-4 pt-4 px-4" key={exp.id}>
                                    <img className="w-full" src={exp.imageUrl} alt="image"/>
                                    <div className="px-6 py-2 text-center">
                                        <h1 className="font-bold text-gray-900 text-xl mb-2">{exp.title}</h1>                      
                                        <p className="text-gray-700 text-base">{exp.desc}</p>                        
                                    </div>
